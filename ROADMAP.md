...
- **2026-07-15** — **HDL: Verilog + VHDL for FPGA/ASIC (It.8).** Added **8 synthesizable filters** in
  **both Verilog** (`hdl/verilog/`) **and VHDL** (`hdl/vhdl/`), each a synchronous streaming block:
  `ema_q15`, `moving_avg`, `dc_blocker_q15`, `fir5` (5-tap FIR), `biquad_df1` (2nd-order IIR),
  `median5` (sorting-network rank filter), `slew_limiter`, and `deadband`. Added Python integer golden
  models (`sim/fixed_models.py`) and a cross-language parity harness (`sim/hdl_parity.py`, `make hdl`)
  that runs the same stimulus through Python, Verilog (Icarus), VHDL (GHDL), and C, asserting
  **bit-exact** agreement (verified: all 8 filters match across Python/Verilog/VHDL, plus C for the
  EMA, over 200 samples). Covers the FIR, 2nd-order IIR, nonlinear-rank, and signal-conditioning
  classes. An embedded dev can now pick a filter/params in Python and deploy the identical behavior to
  an MCU (C) or fabric (Verilog/VHDL).
